...
function SolarSavePage() {
  ...
  const fetchSolarData = async (data) => {
    try {
      ...
      // Generate graph data 📈
      const avgElectricityUsedPerYear = 10000; // typical US home
      const solarKWhPerYear = response.data.systemSizeKW * response.data.avgSunHoursPerDay * 365 * 0.8; // 80% panel efficiency
      const generatedGraphData = generateElectricityReductionData(avgElectricityUsedPerYear, solarKWhPerYear);
  ...
  const generateElectricityReductionData = (avgElectricityUsedPerYear, solarKWhPerYear) => {
    const data = [];
    for (let i = 1; i <= 10; i++) {
      const normalUsage = avgElectricityUsedPerYear * i;
      const reducedUsage = (avgElectricityUsedPerYear - solarKWhPerYear) * i;
      data.push({
        year: `Year ${i}`,
        normalUsage: Math.max(normalUsage, 0),
        reducedUsage: Math.max(reducedUsage, 0),
      });
    }
    return data;
  };
    ...
    <div style={styles.page}>
      ...
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.dropdownContainer}>
          <label htmlFor="systemSize" style={styles.dropdownLabel}>System Size:</label>
          <select
            id="systemSize"
            value={systemSize}
        ...
        <button type="submit" style={styles.button}>Get Solar Data</button>
      </form>
      ...
      {solarData && (
        <div style={styles.resultBox}>
          ...
          <div style={{ marginTop: "2rem" }}>
            <h3>📈 Electricity Usage Over 10 Years</h3>
            <ResponsiveContainer width="100%" height={400}>
              ...
              >
                ...
                <XAxis 
                  dataKey="year"
                  interval={0}  // Force every year label to show
                  tick={{ fontSize: 12 }} // Smaller font if needed
                />
                <YAxis />
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="normalUsage" stroke="#8884d8" name="Without Solar" />
                <Line type="monotone" dataKey="reducedUsage" stroke="#82ca9d" name="With Solar" />
              </LineChart>
            </ResponsiveContainer>
          </div>
...
const styles = {
  page: {
    minHeight: "100vh",
    background: "linear-gradient(to top right, #000000 70%, #f8b500 100%)",
    padding: "2rem",
